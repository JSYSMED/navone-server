...
//   POST /external/v2/standard-group-products   그룹상품 등록
//   GET  /external/v2/standard-group-products   그룹상품 목록 조회
//
// 상품 조회:
...
export const GROUP_API = {
  ...
  groupProducts: "/external/v2/standard-group-products",
};
...
export async function suggestGroups(store, { useAI = true, maxGroups = 20 } = {}) {
  ...
  await enrichImages(store, suggestions);
  return { scanned: products.length, candidates: suggestions };
}

// 후보 상품 대표이미지 보강 — origin-products 단건조회로 representativeImage 채움.
// 후보에 든 상품만(보통 수십 개) 조회, 같은 originProductNo는 1회만(캐시).
async function enrichImages(store, suggestions) {
  const cache = new Map();
  for (const g of suggestions) {
    for (const p of g.products) {
      ...
      if (!cache.has(no)) {
        try {
          const raw = await callCommerce(store, `/external/v2/products/origin-products/${encodeURIComponent(no)}`);
          cache.set(no, raw?.originProduct?.images?.representativeImage?.url || "");
        } catch { cache.set(no, ""); }
      }
      p.image = cache.get(no) || "";
    }
  }
}
...
// POST /external/v2/standard-group-products
// @param {object} store
...
// GET /external/v2/standard-group-products
// ---------------------------------------------
